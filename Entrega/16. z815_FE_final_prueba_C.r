...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , mi_fe1         := mtarjeta_visa_consumo + mtarjeta_master_consumo ]
  dataset[ , mi_fe2         := mtarjeta_visa_consumo / mtarjeta_master_consumo ]
  dataset[ , mi_fe3         := mtarjeta_visa_consumo / mtarjeta_master_consumo ]
  dataset[ , mi_fe4         := mpayroll / cpayroll_trx ]
  dataset[ , mi_fe5         := mprestamos_personales + mplazo_fijo_pesos ]
  dataset[ , mi_fe6         := mprestamos_personales + mplazo_fijo_pesos ]
  dataset[ , mi_fe7         := mcuentas_saldo + mprestamos_personales ]
  dataset[ , mi_fe8         := mcuentas_saldo / mprestamos_personales ]
  dataset[ , mi_fe9         := mpasivos_margen + mcaja_ahorro ]
  dataset[ , mi_fe10         := mpasivos_margen / mcaja_ahorro ]
...
CanaritosAsesinos  <- function( canaritos_ratio=0.2 )
{
  ...
  fwrite( tb_importancia, 
          file= paste0( "impo_prueba_C", GVEZ ,".txt"), #OJO!! Modificar esto
          sep= "\t" )
...
dataset[ , paste0( cols_lagueables, "_lag2") := shift(.SD, 1, NA, "lag"), 
         by= numero_de_cliente, 
         .SDcols= cols_lagueables ]
